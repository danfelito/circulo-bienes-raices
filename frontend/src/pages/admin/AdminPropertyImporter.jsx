...
import { analyzePropertyDocuments, suggestPropertyTitle, emptyPropertyDraft, FIELD_LABELS, validatePropertyDraft } from '../../../../shared/property-metadata.mjs';
...
const AdminPropertyImporter = () => {
  ...
  const [readiness, setReadiness] = useState({ checking: true, ready: false, message: '' });

  const checkConnection = async signal => {
    setReadiness({ checking: true, ready: false, message: '' });
    try {
      await api.getImportReadiness(signal);
      setReadiness({ checking: false, ready: true, message: 'Servicio de carga preparado. Selecciona una carpeta por inmueble.' });
    } catch (error) {
      if (error.name !== 'AbortError') setReadiness({ checking: false, ready: false, message: error.message });
    }
  };
  useEffect(() => {
  ...
  const applyAnalysis = (nextInventory, docs, sourceDocument) => {
    const next = analyzePropertyDocuments(nextInventory, docs, { sourceDocument });
    const title = suggestPropertyTitle(next.draft);
    next.review.originalTitle = next.draft.title;
    next.review.suggestedTitle = title;
    setAnalysis(next); setDraft({ ...emptyDraft, ...next.draft, title, published: false });
  ...
  return <main className="pt-20 min-h-screen bg-[#0a0a0a]">
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex justify-between gap-4 mb-7"><div><p className="text-amber-400 text-sm font-semibold flex items-center gap-2"><Sparkles size={16} /> Importación segura</p><h1 className="text-3xl font-bold text-white mt-1">Crear propiedad desde archivos</h1><p className="text-gray-400 mt-2 max-w-3xl">Selecciona la carpeta del inmueble. Leeremos su ficha y prepararemos el anuncio para que lo revises antes de cargarlo.</p></div><Link to="/admin/propiedades" className="text-sm text-gray-400 hover:text-white">Volver</Link></div>
      <div className="mb-5 p-4 rounded-xl border border-white/15 bg-white/5 text-sm text-gray-200">
        <p>{readiness.checking ? 'Comprobando el servicio de carga…' : readiness.message}</p>
        {!readiness.ready && !readiness.checking && <div className="flex gap-4 mt-3">
          <button type="button" disabled={busy} onClick={() => checkConnection()} className="text-amber-400">Comprobar de nuevo</button>
          <Link to="/admin/login" state={{ from: '/admin/propiedades/importar' }} className="text-amber-400">Iniciar sesión</Link>
        </div>}
        <p className="text-gray-400 mt-2">Importador 2026-09-05.1 · No necesitas abrir Cloudflare ni ejecutar comandos para subir propiedades.</p>
      </div>
      {documents.length > 1 && <label className="block text-sm text-gray-300 mb-5">
        Ficha de información del inmueble
        <select className={inputClass} disabled={busy} value={analysis?.review?.sourceDocument || ''} onChange={event => {
          setError('');
          try { applyAnalysis(inventory, documents, event.target.value); } catch (error) { setError(error.message); }
        }}>
          <option value="">Selecciona la ficha correcta</option>
          {documents.map(document => <option key={document.name} value={document.name}>{document.name}</option>)}
        </select>
        <span className="block mt-2 text-gray-400">Al elegir una ficha se leen únicamente sus datos; revisa los cambios antes de registrar.</span>
      </label>}
      ...
      {result ? <section className="p-8 bg-green-500/10 border border-green-500/25 rounded-2xl text-center"><CheckCircle2 size={48} className="text-green-400 mx-auto" /><h2 className="text-2xl font-bold text-white mt-4">Importación completada</h2><p className="text-gray-300 mt-2">{result.summary.images} fotografías y {result.summary.videos} videos. {result.summary.published ? 'La propiedad ya es visible.' : 'Quedó oculta para revisión.'}</p><div className="flex justify-center gap-3 mt-6"><Link to={`/admin/propiedades/${result.property.id}/editar`} className="px-5 py-3 bg-amber-500 text-white rounded-lg">Revisar propiedad</Link><button type="button" onClick={reset} className="px-5 py-3 text-gray-300">Importar otra</button></div></section> : !analysis ? <div className="grid lg:grid-cols-[1.3fr_.7fr] gap-6">
        ...
        <aside className="space-y-4"><div className="p-5 bg-white/5 rounded-2xl text-sm text-gray-400"><h3 className="text-white font-semibold mb-3">Resultado de lectura</h3><p>{analysis.review?.visualSummary}</p>{analysis.review?.sourceDocument && <p className="mt-3 text-gray-300 break-all">Ficha principal: {analysis.review.sourceDocument}</p>}<p className="mt-3 text-gray-300">Título de la ficha: {analysis.review?.originalTitle || 'No indicado'}</p><p className="mt-3">El título se propone a partir del tipo, superficie y características de la ficha. Puedes editarlo.</p></div>{analysis.review?.warnings?.length > 0 && <div className="p-5 border border-amber-400/25 bg-amber-400/10 rounded-2xl"><h3 className="text-amber-300 font-semibold text-sm">Datos que requieren revisión</h3><ul className="mt-2 space-y-2 text-xs text-gray-300 list-disc pl-4">{analysis.review.warnings.map(warning => <li key={warning}>{warning}</li>)}</ul></div>}<div className="p-5 bg-white/5 rounded-2xl text-sm text-gray-400"><h3 className="text-white font-semibold mb-3">Registro final</h3><p className="flex justify-between">Archivos multimedia <strong className="text-white">{mediaTotal}</strong></p><p className="flex justify-between mt-2">Tamaño total <strong className="text-white">{formatBytes(inventory.totalBytes)}</strong></p><p className="mt-4">Si una carga falla, no se crea la propiedad. Los reintentos reutilizan el mismo identificador y la cancelación programa limpieza segura.</p></div>{!requiredReady && <p className="text-red-300 text-sm flex gap-2"><AlertTriangle size={17} /> Revisa: {pendingLabels}.</p>}{!readiness.ready && !readiness.checking && <p className="text-amber-300 text-sm flex gap-2"><AlertTriangle size={17} className="shrink-0" /> <span>{readiness.message} Puedes pulsar cargar para comprobarlo nuevamente.</span></p>}<button type="button" disabled={busy || !requiredReady} onClick={startImport} className="w-full py-3.5 bg-amber-500 text-white rounded-xl disabled:opacity-40 flex justify-center gap-2">{busy ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle2 size={18} />} Cargar y registrar</button>{error && <button type="button" disabled={busy || !requiredReady} onClick={startImport} className="w-full py-3 bg-white/5 border border-white/10 text-white rounded-xl flex justify-center gap-2"><RotateCcw size={17} /> Reintentar</button>}<p className="text-xs text-gray-500 text-center">La propiedad se publica solo si marcas la opción y todos los archivos se registran correctamente.</p></aside></div>}
    </div>
  </main>;
};
