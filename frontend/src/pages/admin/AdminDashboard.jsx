...
import { ArrowRight, Building2, Eye, LogOut, Mail, Star, TrendingUp, Users } from 'lucide-react';
import api from '../../api';
    ...
    <main className="pt-20 min-h-screen bg-[#0a0a0a]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-2xl font-bold text-white">Dashboard</h1><p className="text-gray-400 text-sm">Panel de administración</p></div>
          <button type="button" onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-500/10 text-red-400 rounded-lg"><LogOut size={16} /> Cerrar Sesión</button>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {statCards.map(({ icon: Icon, label, value, color }) => <div key={label} className="p-4 bg-white/5 rounded-2xl border border-white/5"><Icon className={`${color} mb-2`} size={24} /><p className="text-2xl font-bold text-white">{value}</p><p className="text-xs text-gray-400">{label}</p></div>)}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <Link to="/admin/propiedades/nueva" className="flex items-center gap-3 p-4 bg-amber-400/10 border border-amber-400/20 rounded-xl text-amber-400"><Building2 size={20} /> Nueva Propiedad <ArrowRight size={16} className="ml-auto" /></Link>
          <Link to="/admin/propiedades" className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl text-gray-300"><Building2 size={20} /> Gestionar Propiedades <ArrowRight size={16} className="ml-auto" /></Link>
          ...
          <div className="bg-white/5 rounded-2xl border border-white/5 overflow-hidden">
            {recentProperties.length ? recentProperties.map(property => (
              <div key={property.id} className="flex items-center gap-4 p-4 border-b border-white/5 last:border-0">
                <img src={property.photos?.[0]?.url || '/images/placeholder.svg'} alt={property.title} className="w-12 h-12 rounded-lg object-cover" />
                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-white truncate">{property.title}</p><p className="text-xs text-gray-400">{property.city} · {property.operation === 'venta' ? 'Venta' : 'Renta'} · ${property.price.toLocaleString('es-MX')}</p></div>
                <span className="text-xs px-2 py-1 rounded bg-white/5 text-gray-400">{property.status}</span>
              </div>
          ...
          <div className="bg-white/5 rounded-2xl border border-white/5 overflow-hidden">
            {recentInquiries.length ? recentInquiries.map(inquiry => (
              <div key={inquiry.id} className="flex items-center gap-4 p-4 border-b border-white/5 last:border-0">
                ...
                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-white truncate">{inquiry.name}</p><p className="text-xs text-gray-400 truncate">{inquiry.message}</p></div>
