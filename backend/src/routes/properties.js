...
const isVideoRecord = photo => photo?.publicId?.startsWith('video:') || /\.(mp4|mov|m4v|webm|avi|mkv)(?:\?|$)/i.test(photo?.url || '');
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (existing) propertyData.slug = `${propertyData.slug}-${Date.now()}`;
...
// POST /api/properties/:id/photos - Upload photos and videos
router.post('/:id/photos', authMiddleware, upload.array('photos', 40), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    let hasMainImage = existingPhotos.some(photo => photo.isMain && !isVideoRecord(photo));
    ...
    for (const file of req.files || []) {
      ...
      const isMain = !isVideo && !hasMainImage;
      const photo = await prisma.photo.create({
        data: {
          url: result.secure_url,
          publicId: isVideo ? `video:${result.public_id}` : result.public_id,
          alt: isVideo ? `Video de ${property.title}` : property.title,
          order: nextOrder++,
          isMain,
          propertyId: id,
        },
    ...
    res.status(500).json({ error: 'Error al subir fotografías o videos' });
  }
});
...
router.delete('/:propertyId/photos/:photoId', authMiddleware, async (req, res) => {
  try {
    ...
    if (!photo) return res.status(404).json({ error: 'Archivo no encontrado' });
    ...
    if (photo.isMain) {
      const replacement = await prisma.photo.findFirst({
        where: { propertyId: photo.propertyId, NOT: { publicId: { startsWith: 'video:' } } },
        orderBy: { order: 'asc' },
      });
      if (replacement) await prisma.photo.update({ where: { id: replacement.id }, data: { isMain: true } });
    }
...
router.patch('/:propertyId/photos/:photoId/main', authMiddleware, async (req, res) => {
  try {
    ...
    const photo = await prisma.photo.findUnique({ where: { id: photoId } });
    if (!photo || photo.propertyId !== propertyId) return res.status(404).json({ error: 'Fotografía no encontrada' });
    if (isVideoRecord(photo)) return res.status(400).json({ error: 'Un video no puede usarse como fotografía principal' });

    await prisma.$transaction([
      prisma.photo.updateMany({ where: { propertyId, isMain: true }, data: { isMain: false } }),
      prisma.photo.update({ where: { id: photoId }, data: { isMain: true, order: 0 } }),
    ]);
