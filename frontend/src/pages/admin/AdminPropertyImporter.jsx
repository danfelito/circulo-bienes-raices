...
const emptyDraft = {
  ...
  state: 'Veracruz', country: 'México', address: '', features: [], status: 'available',
  featured: false, published: false, mainPhotoFilename: '',
};
...
const AdminPropertyImporter = () => {
  ...
  return <main className="pt-20 min-h-screen bg-[#0a0a0a]">
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      </div> : <div className="grid xl:grid-cols-[1fr_360px] gap-6"><div className="space-y-6"><section className="p-6 bg-white/5 rounded-2xl"><div className="flex justify-between mb-5"><div><h2 className="text-xl text-white font-semibold">Borrador</h2><p className="text-sm text-gray-400">Confirma los datos antes del registro final.</p></div><button type="button" onClick={() => setAnalysis(null)} className="text-sm text-amber-400">Cambiar archivos</button></div><div className="grid md:grid-cols-2 gap-4">{textFields.map(([name, label, type]) => <label key={name} className="text-xs text-gray-400">{label}<input name={name} type={type} value={draft[name] ?? ''} onChange={handleChange} className={inputClass} /></label>)}</div><label className="text-xs text-gray-400 block mt-4">Descripción *<textarea name="description" rows={6} value={draft.description} onChange={handleChange} className={`${inputClass} resize-y`} /></label><label className="text-xs text-gray-400 block mt-4">Características<input value={featuresText} onChange={event => setFeaturesText(event.target.value)} className={inputClass} placeholder="Alberca, jardín, seguridad" /></label><div className="grid sm:grid-cols-3 gap-4 mt-4"><label className="text-xs text-gray-400">Operación<select name="operation" value={draft.operation} onChange={handleChange} className={inputClass}><option value="venta">Venta</option><option value="renta">Renta</option></select></label><label className="text-xs text-gray-400">Tipo<select name="type" value={draft.type} onChange={handleChange} className={inputClass}>{['casa','departamento','terreno','oficina','local','bodega','rancho','otros'].map(value => <option key={value}>{value}</option>)}</select></label><label className="text-xs text-gray-400">Estado<select name="status" value={draft.status} onChange={handleChange} className={inputClass}><option value="available">Disponible</option><option value="reserved">Reservada</option><option value="sold">Vendida</option><option value="rented">Rentada</option></select></label></div><div className="flex gap-6 mt-5 text-sm text-gray-300"><label><input type="checkbox" name="featured" checked={draft.featured} onChange={handleChange} /> Destacada</label><label><input type="checkbox" name="published" checked={draft.published} onChange={handleChange} /> Hacer visible sólo al completar</label></div></section>
